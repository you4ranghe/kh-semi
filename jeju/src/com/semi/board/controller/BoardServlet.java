...
public class BoardServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		List<Board> boardList = new BoardService().getBoard(cPage, numPerPage);// cPage와 numPerPage를 기준으로 현제 해당페이지의
																				// 목록 을 가져옴

		int pageBarSize = 5;// 게시판 네비게이터의 목록 크기를 정해줌
		int totalData = new BoardService().getTotalDataCount();// 게시판의 총 게시글 수를 가져옴
		...
		if (pageNo == 1) {
			...
			pageBar = "<li class='page-item'>" + "<a class='page-link text-dark' href='" + request.getContextPath()
					+ "/board?cPage=" + (pageNo - 1) + "' aria-label='Previous'>"
					+ "<span aria-hidden='true'>&laquo;</span>" + "<span class='sr-only'>Previous</span>" + "</a>"
					+ "</li>";
		}
		...
		while (!(pageNo > pageEnd || pageNo > totalPage)) {
			if (cPage == pageNo) {
				...
				pageBar += "<li class='page-item'><a class='page-link text-dark' href='" + request.getContextPath()
						+ "/board?cPage=" + pageNo + "'>" + pageNo + "</a></li>";
			}
			pageNo++;
		}
		if (pageNo > totalPage) {
			...
			pageBar += "<li class='page-item'>" + "<a class='page-link text-dark' href='" + request.getContextPath()
					+ "/board?cPage=" + pageNo + "' aria-label='Next'>" + "<span aria-hidden='true'>&raquo;</span>"
