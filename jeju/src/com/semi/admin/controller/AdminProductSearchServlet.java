...
/**
 * Servlet implementation class AdminProductSearchServlet
 */
...
public class AdminProductSearchServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<Product> list=new AdminService().selectProductSearch(type,keyword,cPage,numPerPage);
		System.out.println("검색타입에 따른 list : "+list);
		//검색키워드 타입에 다른 상품객체 총 수 조회 서비스
		int totalData=new AdminService().selectProductSearchCount(type,keyword);
		...
		if(pageNo==1) {
			...
			pageBar="<a href='"+request.getContextPath()
					+ "/admin/productSearch?cPage="+(pageNo-1)
		...
		while(!(pageNo>pageEnd||pageNo>totalPage)) {
			if(pageNo==cPage) {
				...
				pageBar+="<a href='"+request.getContextPath()
				+"/admin/productSearch?cPage="+pageNo
				+"&searchType="+type+"&searchkeyword="+keyword
