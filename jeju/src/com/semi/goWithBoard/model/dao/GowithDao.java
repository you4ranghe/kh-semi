...
public class GowithDao {
	...
	public List<Gowith> getGowith(int cPage, int numPerPage, Connection conn) {
		...
		try {
			...
			while (rs.next()) {
				Gowith g = new Gowith(rs.getInt("GOWITH_NO"), rs.getString("USER_ID"), rs.getString("GOWITH_TITLE"),
						rs.getDate("WRITE_DATE"), rs.getDate("TRAVEL_START"), rs.getDate("TRAVEL_END"),
						rs.getDate("RECRUITMENT_END"), rs.getString("SCHEDULE"), rs.getString("DESTINATION"),
						rs.getInt("TOTAL_PERSON"), rs.getInt("RECRUITMENT_PERSON"), rs.getInt("NOW_GENDER_M"),
						rs.getInt("NOW_GENDER_F"), rs.getInt("RECRUITMENT_GENDER_M"), rs.getInt("RECRUITMENT_GENDER_F"),
						rs.getString("CONTACT"), rs.getString("COMMENTS"), rs.getInt("G_RECOMMEND"),
						rs.getInt("G_HITS"));
				gowithList.add(g);
			}
	...
	public int delete(int gowithNo, Connection conn) {
		PreparedStatement pstmt = null;
		int check = 0;
		System.out.println("dao");
		try {
			System.out.println("첫번째 try");
			pstmt=conn.prepareStatement(prop.getProperty("deleteGcomment"));
			pstmt.setInt(1, gowithNo);
			check = pstmt.executeUpdate();
			System.out.println("첫번째 try 완료");
		} catch (SQLException e) {
			e.printStackTrace();
		} finally {
			close(pstmt);
	...
	public int deleteGFile(int gowithNo, Connection conn) {
		PreparedStatement pstmt = null;
		int check = 0;
		System.out.println("dao");
		try {
			System.out.println("두번째 try");
			pstmt=conn.prepareStatement(prop.getProperty("deleteGFile"));
			pstmt.setInt(1, gowithNo);
			check = pstmt.executeUpdate();
			System.out.println("두번째 try 완료");
		} catch (SQLException e) {
			e.printStackTrace();
		} finally {
			close(pstmt);
	...
	public int deleteGhits(int gowithNo, Connection conn) {
		PreparedStatement pstmt = null;
		int check = 0;
		System.out.println("dao");
		try {
			System.out.println("새번째 try");
			pstmt=conn.prepareStatement(prop.getProperty("deleteGhits"));
			pstmt.setInt(1, gowithNo);
			check = pstmt.executeUpdate();
			System.out.println("세번째 try 완료");
		} catch (SQLException e) {
			e.printStackTrace();
		} finally {
			close(pstmt);
	...
	public int deleteGowith(int gowithNo, Connection conn) {
		PreparedStatement pstmt = null;
		int check = 0;
		System.out.println("dao");
		try {
			System.out.println("네번째 try");
			pstmt=conn.prepareStatement(prop.getProperty("deleteGowith"));
			pstmt.setInt(1, gowithNo);
			check = pstmt.executeUpdate();
			System.out.println("네번째 try 완료");
		} catch (SQLException e) {
			e.printStackTrace();
		} finally {
			close(pstmt);
